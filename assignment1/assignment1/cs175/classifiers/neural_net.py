...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    #we can choose activation function to be RELU
    relu_activation=np.maximum(0,np.dot(X,W1)+b1)
    scores = np.dot(relu_activation, W2)+b2
    ...
    #############################################################################
    #computing the gradient using chain rule and back propagation 
    dscores = h
    dscores[np.arange(N),y] -= 1
    dscores /= N

    grads['W2'] = np.dot(relu_activation.T,dscores)
    grads['b2'] = np.sum(dscores,axis=0,keepdims=True)

    # Backpropagate the gradient to the hidden layer
    dhidden = np.dot(dscores,W2.T)

    # Backprpagate the ReLu
    dhidden[relu_activation==0] = 0

    # Backpropagate the gradient to W and b (input layer)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      #choosing random batch of data and labels
      indx_batch = np.random.choice(np.arange(num_train),size=batch_size)
